...
public class Level9_Scene : IScene
{
    private Game1 _game;
    ...
    //ICE stuff
    private Texture2D _dangerOverlayTexture;
    private Texture2D _speedMaskTexture;
    private Color[] _speedMaskPixels;

    


    public Level9_Scene(Game1 game)
    {
        _game = game;
    }
    ...
    public void LoadContent(ContentManager content)
    {
        ...
        _dangerOverlayTexture = content.Load<Texture2D>("dangerZoneMap2-overlay");
        _speedMaskTexture = content.Load<Texture2D>("dangerZoneMap21"); 
    ...
    public void Update(GameTime gameTime)
    {
        ...
        {
            _player.ApplySlow(0.2f);
        }
        // if (IsInSlowZone(_player._position))
        // {
        //     _player.ApplySlow(1f);
        // }
    ...
    public void Draw(SpriteBatch spriteBatch)
    {
        ...
        spriteBatch.Draw(_dangerOverlayTexture, new Rectangle(0, 0, 1920, 1080), Color.White * 0.8f); // adjust alpha if needed
    ...
    private void DrawDebugCircle(SpriteBatch spriteBatch, Vector2 position, float radius, Color color)
    {
        int size = (int)(radius * 2);
        Rectangle rect = new Rectangle((int)(position.X - radius), (int)(position.Y - radius), size, size);
        spriteBatch.Draw(_circleTexture, rect, color * 0.5f); // 50% transparent
    }
    ...
    public void SaveGameState()
    {
        ...
        data.LastCompletedLevel = 9; // This scene = Level 1
    ...
    private void HandlePauseMenuInput()
    {
        ...
        if (mouseState.LeftButton == ButtonState.Pressed && _previousMouseState.LeftButton == ButtonState.Released)
        {
            ...
            else if (_settingsButton.Contains(mouseState.Position))
            {
                ...
                settings.SetCaller(SceneManager.SceneType.Level9);
    ...
    private bool IsInSlowZone(Vector2 position)
    {
        if (_speedMaskPixels == null)
            return false;

        int texWidth = _speedMaskTexture.Width;
        int texHeight = _speedMaskTexture.Height;

        // Convert position to pixel coords
        int x = (int)MathHelper.Clamp(position.X / 1920f * texWidth, 0, texWidth - 1);
        int y = (int)MathHelper.Clamp(position.Y / 1080f * texHeight, 0, texHeight - 1);

        // Read from cached array
        Color pixel = _speedMaskPixels[y * texWidth + x];

        // If pixel is black → not slowed
